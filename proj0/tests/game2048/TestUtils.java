...
public class TestUtils {
    ...
    public static void checkModel(int[][] values, int score, int maxScore,
        ...
        assertWithMessage(errMsg).that(model).isEqualTo(expected);
    }
    ...
    public static void checkChanged(Side s, boolean expected, boolean actual) {
        ...
        if (expected) {
            ...
            assertWithMessage(changedErrMsg).that(actual).isTrue();
        } else {
            changedErrMsg = boardShouldNotChange(s);
            assertWithMessage(changedErrMsg).that(actual).isFalse();
        }
    }
}
